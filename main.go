...
type HangManData struct {
	Word             string // MOt avec les tirets
	ToFind           string // Mot choisi par le programme
	Attempts         int    // Nombre de vies restantes
	HangmanPositions string // Positions de Hangman.txt
...
func FindaWord() string { // la string qu'on va return sera un mot aléatoirement choisi dans words.txt
...
func main() {
	...
	hangman.ToFind = FindaWord()            // sinon on initialise le jeu à 0 avec la fonction motatrouver expliquée plus haut (elle trv un mot o pif dans words.txt qui va etre celui à trouver)
	...
	tmpl, _ := template.ParseGlob("./html_page/*.gohtml")
	...
	fmt.Println("(http://localhost:999) - server start on port", port)
	...
	http.ListenAndServe("localhost:999", nil) // ici on héberge notre serveur en local sur le port 999.
